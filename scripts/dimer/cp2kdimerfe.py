...
for opt_name,opt_value in opts:
    if opt_name in ('-h','--help'):
        print('''A script written to read the out put of Transition state search using dimer method. 
''')
        exit()
...
while (i < len(lines)):
    if ( re.search(r"ENERGY| Total FORCE_EVAL",lines[i])):
    ...
    elif (re.search(r"Informations at step",lines[i])):
        ...
        if (maxforce > 0.00):
            ...
            outlines += "%d\t%12.8f\n" %(stepnum, energy)
    elif (re.search(r"GEOMETRY OPTIMIZATION COMPLETED", lines[i])):
        stepnum += 1
        converge = 1 
        while (i < len(lines)) :
            if ( re.search(r"ENERGY| Total FORCE_EVAL",lines[i])):
